...
<?php
    ...
    if($sqltable == 'stock_tbl') {
        if($sqlcondition == null) {
            $sqlstatement = "SELECT $sqlfield FROM $sqltable;";
        } else {
            $sqlstatement = "SELECT $sqlfield FROM $sqltable WHERE $sqlcondition;";
        }
        $sqlquery = mysql_query($sqlstatement, $connection);
        $sqlcount = mysql_num_rows($sqlquery);

        echo "SQL Statement: $sqlstatement<br>";
        echo "Number of Records: $sqlcount<br>";
        ...
        while($row = mysql_fetch_assoc($sqlquery, MYSQL_ASSOC)) {
            echo "<tr>";
